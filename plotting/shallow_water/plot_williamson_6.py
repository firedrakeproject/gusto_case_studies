"""
Plots the Williamson 6 jet test case.
"""
...
from tomplot import (
    ...
    add_colorbar_ax, add_colorbar_fig, plot_field_quivers, tomplot_field_title,
...
results_file_name = f'{abspath(dirname(__file__))}/../../results/williamson_6/field_output.nc'
plot_stem = f'{abspath(dirname(__file__))}/../../figures/shallow_water/williamson_6'
...
init_contours = [np.linspace(0.0, 100.0, 11),
                 np.linspace(8000.0, 10600.0, 14),
                 np.linspace(-1e-4, 1e-4, 17)]

# ---------------------------------------------------------------------------- #
# Final plot details
# ---------------------------------------------------------------------------- #
final_field_names = ['RelativeVorticity', 'D']
final_colour_schemes = ['RdBu_r', 'YlGnBu']
final_field_labels = [r'$\zeta$ (s$^{-1}$)', r'$D$ (m)']
final_contours_to_remove = [0.0, None]
final_contours = [np.linspace(-1e-4, 1e-4, 17),
                  np.linspace(8000.0, 10600.0, 14)]
time_idxs = [1, 7, 14]
...
for i, (ax, field_name, colour_scheme, field_label, contour_to_remove, contours) in \
    ...
    if field_name == 'u':
        ...
        plot_field_quivers(
            ...
            spatial_filter_step=(4, 2), magnitude_filter=1.0,
        )
...
# FINAL PLOTTING
# ---------------------------------------------------------------------------- #
fig, axarray = plt.subplots(3, 2, figsize=(16, 10), sharex='all', sharey='all')

for j, (axrow, time_idx) in enumerate(zip(axarray, time_idxs)):

    for i, (ax, field_name, colour_scheme, field_label, contour_to_remove, contours) in \
        enumerate(zip(
            axrow, final_field_names, final_colour_schemes,
        ...
        time_title = f't = {time:.1f} days'
        tomplot_field_title(ax, time_title, minmax=True, field_data=field_data)

        # Labels -------------------------------------------------------------------
        if i == 0:
            ax.set_ylabel(r'$\vartheta$ (deg)', labelpad=-10)
            ax.set_ylim(ylims)
            ax.set_yticks(ylims)
            ax.set_yticklabels(ylims)
        ...
        if j == 2:
            ...
            ax.set_xticklabels(xlims)
            add_colorbar_fig(
                fig, cf, field_label, ax_idxs=[i, i+2, i+4],
                location='bottom', cbar_labelpad=-10
            )
