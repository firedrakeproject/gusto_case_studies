...
test = 'vertical_slice_nair_lauritzen_convergence'
...
# General options
# ---------------------------------------------------------------------------- #
rho_contours = np.linspace(0.0, 3.6, 37)
rho_lines = np.array([0.1*i for i in range(10)] + [1.0 + 0.25*i for i in range(15)])
rho_colour_scheme = 'gist_earth_r'
rho_field_label = r'$\rho_d$ (kg m$^{-3}$)'
m_contours = np.linspace(0.01, 0.07, 13)
m_colour_scheme = 'Purples'
m_contour_to_remove = 0.02
...
for i, (ax, time_idx, field_name, cbar) in \
    ...
    if field_name == 'rho_d':
        ...
        cmap, _ = tomplot_cmap(contours, colour_scheme, remove_contour=None)
        lines = rho_lines
        cbar_labelpad = -10
        data_format = '.2f'
        ...
        cmap, lines = tomplot_cmap(contours, colour_scheme, remove_contour=m_contour_to_remove)
        cbar_labelpad = -10
        data_format = '.3e'
    ...
    if cbar:
        add_colorbar_fig(
            ...
            cbar_labelpad=cbar_labelpad, cbar_format='.2f'
        )
    tomplot_field_title(
