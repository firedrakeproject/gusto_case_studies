...
from firedrake import (ExtrudedMesh,  TensorProductElement,
                       ...
                       NonlinearVariationalProblem, NonlinearVariationalSolver, conditional)
from gusto import *          
...
days = 15.
tmax = days * 24. * 60. * 60.
n = 3   # cells per cubed sphere face edge
nlayers = 3 # vertical layers
...
print(f'ideal number of processors = {eqn.X.function_space().dim() / 50000}')
...
diagnostic_fields = [MeridionalComponent('u'), ZonalComponent('u'),RadialComponent('u'),
                    CourantNumber(), Temperature(eqn), CompressibleRelativeVorticity(),
                    MeridionalComponent('CompressibleRelativeVorticity'), 
                    ZonalComponent('CompressibleRelativeVorticity'),
                    RadialComponent('CompressibleRelativeVorticity')
...
# Get spherical basis vectors, expressed in terms of their (x,y,z) components:
