...
def baroclinic(order, c, nlayers, dt, tmax, alpha, variable_height, u_form):
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="The deep atmosphere dry baroclinic wave Test case from Ullich et al. 2013",
        formatter_class=ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        '--horder',
        help="The horizontal order",
        type=int,
        default=1
    )
    parser.add_argument(
        '--vorder',
        help="The vertical order",
        type=int,
        default=1
    )
    parser.add_argument(
        "--no-variable-height",
        help="Toggle variable height off",
        action="store_true"
    )
    ...
    variable_height = (not args.no_variable_height)
    u_form = 'vector_advection_form'
    breakpoint()
    baroclinic(order, c, nlayers, dt, tmax, alpha, variable_height, u_form)
