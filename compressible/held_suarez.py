...
dirname = f'relaxation_printing3'
...
exner = exner_pressure(params, rho_averaged, theta0)
exner_field = Function(Vt).interpolate(exner)
exner_field_map = exner_field.cell_node_map()

T_condition = (T0surf - T0horiz * sin(lat)**2 - T0vert * ln(exner) * cos(lat)**2 / kappa) * exner
...
Vflat = FunctionSpace(mesh, Vtflat)

#extracts the bottom values into a flat function space with same propertes
exner_surface = Function(Vflat)
exner_surface_mask = Vt.finat_element.entity_dofs()[(2, 0)][0]
exner_surface.dat.data[exner_surface.cell_node_map().values] =  \
exner_field.dat.data_ro[exner_field_map.values[:, exner_surface_mask]]

# interpolate exener_field / exner_surface into a sigma space 
sigma = Function(Vt).interpolate(exner_field / exner_surface)
...
forcing_field=stepper.fields('forcing', space=Vt, dump=True)
sigma_field=stepper.fields('Sigma', space=Vt, dump=True)
ic('printing_forcing')
sigma_field.interpolate(exner_field / exner_surface)
forcing_field.interpolate(newton_freq * equilibrium_expr)
